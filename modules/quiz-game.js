...
function renderQuizQuestion() {
  ...
  if (optionsEl) {
    optionsEl.innerHTML = question.options
      .map(
        (option, idx) =>
          `<div class="quiz-option" data-index="${idx}"><span class="option-letter">${String.fromCharCode(65 + idx)}</span><span class="option-text">${option}</span></div>`
      )
      .join('');
    optionsEl
...
function finishQuiz() {
  ...
  if (
    !userProgress.bestQuizTimes[topicKey] ||
    completionTime < userProgress.bestQuizTimes[topicKey]
  )
