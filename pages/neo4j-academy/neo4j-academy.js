...
let userProgress = JSON.parse(localStorage.getItem('neo4jHubProgress')) || { completedLessons: [], completedQuizzes: [] };
...
function renderQuiz(mIndex) {
    ...
    quiz.forEach((q, i) => {
        ...
        html += \`
                ...
                <button onclick="checkAnswer('\${q.id}', \${mIndex}, \${i})" class="mt-4 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded text-sm font-medium transition-colors">
                    Submit Answer
                </button>
...
function runSimulation() {
    ...
    if (relMatch) {
        ...
        matchedEdges.forEach(edge => {
            if (!matchedNodes.find(n => n.id === edge.from)) matchedNodes.push(mockGraphData.nodes.find(n => n.id === edge.from));
            if (!matchedNodes.find(n => n.id === edge.to)) matchedNodes.push(mockGraphData.nodes.find(n => n.id === edge.to));
        });
    }
    ...
    elements.jsonResults.innerHTML = JSON.stringify(resultObj, null, 2).replace(/"([^"]+)":/g, '<span class="text-indigo-600">"$1"</span>:');
