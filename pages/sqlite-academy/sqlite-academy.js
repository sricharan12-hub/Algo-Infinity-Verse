...
let userProgress = JSON.parse(localStorage.getItem('sqliteHubProgress')) || { completedLessons: [], completedQuizzes: [] };
        ...
        quiz: [
            {
                ...
                options: ['Returns all rows from both tables', 'Returns only rows that have matching values in both tables', 'Returns all rows from the left table', 'Deletes matching rows'],
                correct: 1
            }
        ]
    },
...
function renderQuiz(mIndex) {
    ...
    quiz.forEach((q, i) => {
        ...
        html += \`
                ...
                <button onclick="checkAnswer('\${q.id}', \${mIndex}, \${i})" class="mt-4 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded text-sm font-medium transition-colors">
                    Submit Answer
                </button>
...
function runQuery() {
    if (!db) {
        elements.resultsPane.innerHTML = '<div class="error-msg">Database not initialized yet. Please wait.</div>';
        return;
    }

    const query = elements.sqlEditor.value.trim();
    if (!query) {
        elements.resultsPane.innerHTML = '<div class="absolute inset-0 flex items-center justify-center text-gray-400 italic">Please enter a SQL query.</div>';
        return;
    }
    ...
    try {
        ...
        if (results.length === 0) {
            ...
            elements.resultsPane.innerHTML = '<div class="p-4 text-green-600 font-medium"><i class="fas fa-check-circle mr-2"></i>Query executed successfully (no results to display).</div>';
            return;
        }
